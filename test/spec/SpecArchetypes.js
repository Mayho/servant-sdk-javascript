...
describe("Test Servant SDK Archetype Functions --- ", function() {
	...
	it("Check Servant SDK Is Installed Correctly", function() {
        expect(Servant.status).toBe('uninitialized');
    });

    it("Initialize Servant SDK", function() {
        Servant.initialize({
            application_client_id: '9897668a5f986asf9a85asf'
        }, function(status) {
        	expect(status).toBe('no_token');
        });
    });

    // it("Instantiate Product", function() {
    //     var product = Servant.instantiate('product');
    //     expect(product).not.toEqual(null);
    //     console.log('Create New Instances', product);
    // });
